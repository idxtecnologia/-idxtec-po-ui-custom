...
import { PoPageCustomizationService, PoPageDynamicEditActions, PoPageDynamicEditBeforeCancel, PoPageDynamicEditField, PoPageDynamicEditMetadata, PoPageDynamicEditOptions, PoPageDynamicOptionsSchema, PoPageDynamicService } from '@po-ui/ng-templates';

import { convertToBoolean, mapObjectByProperties, valuesFromObject, removeKeysProperties } from '../utils/util';
import { IdxPoPageDynamicEditActionsService } from './idx-po-page-dynamic-edit-actions.service';
 ...
 */
 @Component({
  selector: 'idx-po-page-dynamic-edit',
  templateUrl: './idx-po-page-dynamic-edit.component.html',
  styleUrls: ['./idx-po-page-dynamic-edit.component.css']
})
export class IdxPoPageDynamicEditComponent implements OnInit, OnDestroy {
   ...
   * onde o usuário poderá customizarPoModalComponent, { static: true} 
   * ```
  ...
  @Input('p-fields') set fields(value: Array<PoPageDynamicEditField>) {
    ...
    this.getInitValues( this._fields );
  }
  ...
  private getInitValues(fields: Array<any> = []) {
    const f = fields.filter(field => field.initValue);
    f.forEach( field => {
      this.model[field.property] = field.initValue
    })
  }
  ...
  constructor(
    ...
    private poPageDynamicEditActionsService: IdxPoPageDynamicEditActionsService,
