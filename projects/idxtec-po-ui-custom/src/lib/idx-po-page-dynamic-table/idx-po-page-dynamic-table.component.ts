 ...
 */
@Component({
  selector: 'idx-po-page-dynamic-table',
  templateUrl: './idx-po-page-dynamic-table.component.html',
  providers: [PoPageDynamicService],
})
export class IdxPoPageDynamicTableComponent
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
   ...
   * ```
   */
  @Input('p-page-custom-actions') set pageCustomActions(
    value: Array<PoPageDynamicTableCustomAction>
  ) {
    this._pageCustomActions = Array.isArray(value) ? value : [];

    this.customPageListActions = this.transformCustomActionsToPageListAction(
      this.pageCustomActions
    );
    this.hasCustomActionWithSelectable = this.pageCustomActions.some(
      (customAction) => customAction.selectable
    );
  }
   ...
   * > Caso utilizar a ação padrão de excluir, a mesma será exibida por último na tabela.
   */
  @Input('p-table-custom-actions') set tableCustomActions(
    value: Array<PoPageDynamicTableCustomTableAction>
  ) {
    this._tableCustomActions = Array.isArray(value) ? value : [];

    this.customTableActions = this.transformTableCustomActionsToTableActions(
  ...
  onQuickSearch(termTypedInQuickSearch) {
    const quickSearchParam = termTypedInQuickSearch
      ...
      : {};
    this.params = this.concatFilters
      ? { ...this.params, ...quickSearchParam }
      : { ...quickSearchParam };

    this.subscriptions.add(
      this.loadData(
        termTypedInQuickSearch ? { page: 1, ...this.params } : undefined
      ).subscribe()
    );
  }
  ...
  onSort(sortedColumn: PoTableColumnSort) {
    ...
    this.params.order = `${sortedColumn.type === 'ascending' ? '' : '-'}${
      sortedColumn.column.property
    }`;

    this.subscriptions.add(
      this.loadData({ page: this.page, ...this.params }).subscribe()
    );
  }
  ...
  refreshData() {
    this.subscriptions.add(this.loadData({ ...this.params }).subscribe());
  }
  ...
  private confirmRemoveAll(
    ...
    const confirmOptions: PoDialogConfirmOptions = {
      ...
      confirm: this.removeAll.bind(
        this,
        actionRemoveAll,
        actionBeforeRemoveAll
      ),
    };
  ...
  private loadData(
    params: { page?: number; search?: string; order?: string } = {}
  ) {
    if (!this.serviceApi) {
    ...
    const defaultParams: any = { page: 1, pageSize: 25 };
  ...
  private getMetadata(
    ...
    if (serviceApiFromRoute) {
      return this.poPageDynamicService
        .getMetadata<PoPageDynamicTableMetaData>()
        .pipe(
          tap((response) => {
            ...
            this.pageCustomActions =
              response.pageCustomActions || this.pageCustomActions;
            this.tableCustomActions =
              response.tableCustomActions || this.tableCustomActions;
            this.keepFilters = response.keepFilters || this.keepFilters;
            this.concatFilters = response.concatFilters || this.concatFilters;
            this.quickSearchWidth =
              response.quickSearchWidth || this.quickSearchWidth;
          }),
  ...
  private navigateTo(
    ...
    this.router
      .navigate([route.url || route.path], { queryParams: route.params })
      .catch(() => {
  ...
  private executeDetail(
    ...
    const allowAction =
      typeof before.allowAction === 'boolean' ? before.allowAction : true;
    const { newUrl } = before;
  ...
  private openDuplicate(
    ...
    const id = this.formatUniqueKey(item);
    const duplicates = util.removeKeysProperties(
      this.keys,
      util.mapObjectByProperties(item, this.duplicates)
    );
  ...
  private executeDuplicate(
    ...
    if (allowAction && actionDuplicate) {
      if (
        typeof beforeDuplicateResource === 'object' &&
        beforeDuplicateResource !== null
      ) {
        duplicates = util.removeKeysProperties(
          this.keys,
          beforeDuplicateResource
        );
      }
      ...
      if (typeof newAction === 'string') {
        return this.navigateTo({
          path: newAction,
          params: { duplicate: JSON.stringify(duplicates) },
        });
      }
  ...
  private executeEditAction(
    ...
    if (typeof newEditAction === 'string') {
      ...
      if (typeof updatedItem === 'object' && updatedItem !== null) {
        this.modifyUITableItem(
          item,
          util.removeKeysProperties(this.keys, updatedItem)
        );
      }
    }
    ...
    this.subscriptions.add(
      this.poPageDynamicTableActionsService
        ...
        .subscribe((beforeNewResult: PoPageDynamicTableBeforeNew) =>
          this.executeNew(actionNew, beforeNewResult)
        )
    );
  }

  private executeNew(
    actionNew: PoPageDynamicTableActions['new'],
    beforeNewResult?: PoPageDynamicTableBeforeNew
  ) {
      ...
      this.poPageDynamicTableActionsService
        ...
        .pipe(
          switchMap((beforeRemove) =>
            this.deleteAction(item, actionRemove, beforeRemove)
          )
        )
        .subscribe()
    );
        ...
        return this.poPageDynamicService
          ...
          .pipe(
            map(() =>
              this.removeFromUI(
                resourceToRemoveKey,
                this.literals.removeSuccessNotification
              )
            )
          );
      }
  ...
  private deleteAllAction(
    ...
    if (allow && Array.isArray(resourcestoDelete)) {
      if (typeof actionRemoveAll === 'boolean' || newUrl) {
        return this.poPageDynamicService
          .deleteResources(resourcestoDelete, newUrl)
          .pipe(
            tap(() => {
              this.removeFromUI(
                resourcestoDelete,
                this.literals.removeAllSuccessNotification
              );
            })
          );
      }
  ...
  private removeLocalItems(itemsKeysToRemove = []) {
    if (itemsKeysToRemove.length) {
      this.items = this.items.filter((item) => {
        ...
        return !itemsKeysToRemove.find(
          (itemKeyToRemove) =>
            util.valuesFromObject(itemKeyToRemove).join('|') === itemKey
        );
      });
    }
  }
  ...
  private callPageCustomAction(customAction: PoPageDynamicTableCustomAction) {
    if (customAction.action) {
      const selectedItems = customAction.selectable
        ? this.getSelectedItemsKeys()
        : undefined;
  ...
  private callTableCustomAction(
    customAction: PoPageDynamicTableCustomTableAction,
    selectedItem
  ) {
    if (customAction.action) {
      const sendCustomActionSubscription = this.poPageDynamicTableActionsService
    ...
    if (this.showRemove(action.removeAll)) {
      this.defaultPageActions = [
        ...this._defaultPageActions,
        {
          ...
          action: this.confirmRemoveAll.bind(
            this,
            action.removeAll,
            action.beforeRemoveAll
          ),
  ...
  private setTableActions(actions: PoPageDynamicTableActions) {
    if (actions) {
      ...
      this.defaultTableActions = [
        ...
        {
          action: this.confirmRemove.bind(
            this,
            actions.remove,
  ...
  private getInitialValuesFromFilter() {
    const initialFilters = this.filters.reduce(
      (result, item) =>
        Object.assign(result, { [item.property]: item.initValue }),
      {}
    );
    ...
    if (onLoad) {
      return this.getPoDynamicPageOptions(onLoad).pipe(
        tap((responsePoOption) =>
          this.poPageCustomizationService.changeOriginalOptionsToNewOptions(
            this,
            responsePoOption
          )
        )
  ...
  private getPoDynamicPageOptions(
    onLoad: UrlOrPoCustomizationFunction
  ): Observable<PoPageDynamicTableOptions> {
    ...
    return this.poPageCustomizationService.getCustomOptions(
      onLoad,
      originalOption,
      pageOptionSchema
    );
  ...
  private updateTableActions() {
    const defaultTableActionsWithoutActionDelete =
      this._defaultTableActions.filter(
    ...
    const newTableActions = [
      ...defaultTableActionsWithoutActionDelete,
